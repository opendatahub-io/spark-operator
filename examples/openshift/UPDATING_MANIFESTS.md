...
- [ ] `deployment.yaml` - check for:
  ...
  - [ ] **NO fsGroup** in securityContext
  - [ ] Updated image tag
- [ ] `manifests.yaml` - **auto-generated by `make manifests`** - Contains MutatingWebhookConfiguration and ValidatingWebhookConfiguration

> **Note:** The `manifests.yaml` file is auto-generated by `controller-gen` and contains combined webhook configurations.

### RBAC (in `config/rbac/`)
- [ ] `clusterrolebinding.yaml` - usually unchanged
- [ ] `leader-election-role.yaml` - namespace-scoped Role for leader election (custom)
- [ ] `role.yaml` - **auto-generated by `make manifests`** - ClusterRole with controller permissions
- [ ] `rolebinding.yaml` - usually unchanged
...
> **Note:** The `role.yaml` file is auto-generated by `controller-gen` during `make manifests`.
...
The following files are auto-generated by `make manifests` (via `controller-gen`) and are the **authoritative source** for their respective configurations:

| File | Generated Content | Kustomize Patches Applied |
|------|-------------------|---------------------------|
| `config/rbac/role.yaml` | ClusterRole `spark-operator-controller` | Adds `apiGroups: [""]` for core resources |
| `config/webhook/manifests.yaml` | MutatingWebhookConfiguration and ValidatingWebhookConfiguration | Adds objectSelector, timeoutSeconds, service name, webhook name |

These files are generated from Go code annotations (`+kubebuilder:rbac` and `+kubebuilder:webhook` markers). To update permissions or webhook configuration:

1. Update the Go annotations in the controller/webhook source files
2. Run `make manifests` to regenerate
3. Update kustomize patches if new customizations are needed
...
│   ├── kustomization.yaml            # Includes patches for apiGroups
│   ├── leader-election-role.yaml     # Namespace-scoped Role for leader election
│   ├── role.yaml                     # Auto-generated ClusterRole (source of truth)
│   ├── rolebinding.yaml
│   ├── scheduledsparkapplication_editor_role.yaml
│   ├── scheduledsparkapplication_viewer_role.yaml
│   ├── spark-application-rbac.yaml   # SA, Role, RoleBinding for Spark drivers
...
└── webhook/
    ...
    ├── clusterrolebinding.yaml
    ├── deployment.yaml
    ├── kustomization.yaml            # Includes patches for objectSelector, etc.
    ├── kustomizeconfig.yaml
    ├── manifests.yaml                # Auto-generated webhooks (source of truth)
    ├── role.yaml
    ├── rolebinding.yaml
    ├── service.yaml
    └── serviceaccount.yaml
